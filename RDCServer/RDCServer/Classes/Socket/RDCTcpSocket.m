...
-(void)connectToHost:(RDCHostInfo *)hostInfo {
    ...
    m_pBufferEvent = bufferevent_socket_new(m_pEventBase, -1, BEV_OPT_CLOSE_ON_FREE);
...
-(void)accept_call_back:(evutil_socket_t)fd addr:(struct sockaddr*)addr
{
    ...
    newSocket->m_pBufferEvent = bufferevent_socket_new(m_pEventBase, fd, BEV_OPT_CLOSE_ON_FREE);
    ...
    if([self.delegate
        respondsToSelector:@selector(clientConnectedWithOnNewSocket:addr:)])
    {
...
-(void)read_call_back {
    ...
    if([self.delegate respondsToSelector:@selector(messageReceivedOnSocket:message:)])
...
-(void)event_dispatch_call_back:(short)what {
    ...
    {
        //连接已断开
        if([self.delegate respondsToSelector:@selector(clientDisconnectedOnSocket:)])
            [self.delegate clientDisconnectedOnSocket:self];
    }
    else if(what & BEV_EVENT_TIMEOUT)
    {
        //超时
        if([self.delegate respondsToSelector:@selector(timeOutEventOccuredOnSocket:)])
