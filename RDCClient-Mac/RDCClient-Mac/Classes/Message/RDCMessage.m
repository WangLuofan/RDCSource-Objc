...
-(void)appendLongInteger:(unsigned long)data {
    [self._data_ appendBytes:&data length:sizeof(unsigned long)];
    return ;
}
...
-(unsigned char)nextChar {
    ...
    if(__data_.length >= sizeof(unsigned char)) {
...
-(unsigned short)nextShort {
    ...
    if(__data_.length >= sizeof(unsigned short)) {
...
-(unsigned int)nextInteger {
    ...
    if(__data_.length >= sizeof(unsigned int)) {
...
-(unsigned long)nextLongInteger {
    unsigned long data = 0;
    
    if(__data_.length >= sizeof(unsigned long)) {
        [__data_ getBytes:&data range:NSMakeRange(_theCurrent, sizeof(unsigned long))];
        _theCurrent += sizeof(unsigned long);
    }
